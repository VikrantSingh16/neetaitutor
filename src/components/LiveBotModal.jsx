...
const LiveBotModal = ({ isOpen, onRequestClose }) => {
  const modalStyles = {
    content: {
      width: "80%",
      maxWidth: "600px",
  ...
  return (
    ...
    <p className='text-sm text-center font-semibold '>You have finished this Topic</p>
    <h2 className='font-semibold text-base text-center mt-2'>Click on next topic from the sidebar to continue the learning.</h2>
    <div className='mt-4'>
    {/* <button className='text-[#FF9A33] border px-7 py-1 rounded-full mr-3' onClick={onRequestClose}>Skip</button> */}
    <button onClick={onRequestClose} className='bg-[#FF9A33] border px-7 py-1 rounded-full text-white' >Back to Learning</button>
    </div>
    </div>
